...
function SideDrawer() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      console.log("SEarching a user form");

      const { data } = await axios.get(
        `/api/admin/chats/search?search=${search}`,
        config
      );
  ...
  const accessChat = async (userId) => {
    try {
      ...
      const { data } = await axios.post("/api/admin/chats", { userId }, config);
      console.log(data);
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <Menu>
            <Link href="/admin">
              <MenuButton p={1}>Home</MenuButton>
            </Link>
          </Menu>
          <Menu>
